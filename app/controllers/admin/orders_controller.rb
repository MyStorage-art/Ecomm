class Admin::OrdersController < AdminController
  ...
  def index
    @not_fullfilled_pagy, @not_fullfilled_orders = pagy(Order.where(full_filled: false).order(created_at: :asc))
    @fullfilled_pagy, @fullfilled_orders = pagy(Order.where(full_filled: true).order(created_at: :asc), page_param: :page_fulfilled)
  end
  ...
  private
    ...
    def admin_order_params
      params.require(:order).permit(:customer_email, :full_filled, :total, :address)
    end
end
